...
deploy_images() {
    ...
    for app in Gateway Product; do
        app_lower=$(echo $app | tr '[:upper:]' '[:lower:]')
        aws ecr describe-repositories --repository-name ${PROJECT_NAME}-${app_lower} >/dev/null 2>&1 || aws ecr create-repository --repository-name ${PROJECT_NAME}-${app_lower}
        docker build -t ${ECR_IMAGE_PREFIX}-${app_lower} ${DIR}/${app} --build-arg GO_PROXY=${GO_PROXY:-"https://proxy.golang.org"}
        aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin ${AWS_ACCOUNT_ID}.dkr.ecr.us-west-2.amazonaws.com
        docker push ${ECR_IMAGE_PREFIX}-${app_lower}
    done
}
...
deploy_infra() {
    ...
    aws cloudformation deploy \
        ...
        --template-file "${DIR}/Infra/base-infra.yaml" \
...
deploy_app() {
    ...
    aws cloudformation deploy \
        ...
        --template-file "${DIR}/Infra/containers.yaml" \
        --capabilities CAPABILITY_IAM \
        --parameter-overrides "ProjectName=${PROJECT_NAME}" "EnvoyImage=${ENVOY_IMAGE}" "GatewayClientImage=${ECR_IMAGE_PREFIX}-gateway" "ProductServerImage=${ECR_IMAGE_PREFIX}-product"
}
...
deploy_mesh() {
    ...
    aws cloudformation deploy \
        ...
        --template-file "${DIR}/Infra/app-mesh.yaml" \
        --capabilities CAPABILITY_IAM \
